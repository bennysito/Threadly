...
<nav class="bg-white">
    <div class="max-w-7xl mx-auto px-4 py-2 flex flex-wrap items-center justify-between gap-2">
        ...
        <div class="hidden md:flex items-center gap-6 ml-4">
            ...
            </a>        <a href="https://samtu43.github.io/Sam_chanel/" class="flex items-center hover:text-amber-500"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
            ...
            <a href="wishlist_panel.php"class="flex items-center text-gray-700 hover:text-amber-500 font-medium"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
            ...
            <div class="relative">
                ...
                <div id="profileDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white border rounded-lg shadow-lg z-10">
                    <?php if ($isLoggedIn): ?>
                        ...
                        <a href="wishlist_panel.php" id="openWishlistDropdownBtn" class="flex items-center px-4 py-2 hover:bg-gray-100 gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
                            </svg>
                            <span>Wishlist</span>
...
<script>
    // Grab the buttons and menus
    const profileBtn = document.getElementById('profileBtn');
    const profileDropdown = document.getElementById('profileDropdown');
    const mobileBtn = document.getElementById('mobileBtn');
    const mobileMenu = document.getElementById('mobileMenu');

    // 1. Toggle Profile Dropdown on click
    profileBtn.addEventListener('click', () => {
        profileDropdown.classList.toggle('hidden');
    });

    // 2. Toggle Mobile Menu on Hamburger click
    mobileBtn.addEventListener('click', () => {
        mobileMenu.classList.toggle('hidden');
    });

    // 3. Optional: close the profile dropdown if user clicks outside
    document.addEventListener('click', (event) => {
        // Check if the click is outside the profile button AND the dropdown
        if (!profileBtn.contains(event.target) && !profileDropdown.contains(event.target)) {
            profileDropdown.classList.add('hidden');
        }
        
        // Optionally: close the mobile menu if clicked outside too (or just rely on re-clicking the button)
